...
def report():
    ...
    supplement_path = OUT / 'zone_supplement.json'
    supplements = json.loads(supplement_path.read_text(encoding='utf-8')) if supplement_path.exists() else []
    supplementary_zones = {(s['site'], s['original_network'], s['resolver'],
                            s['original_time_utc'], s['final_name']): s['zone']
                           for s in supplements if s.get('zone')}
    ...
    for (site, network, resolver), r in sorted(latest.items()):
        ...
        zone = r.get('final_zone')
        if not zone:
            extra = supplementary_zones.get((site, network, resolver, r['time_utc'], r.get('final_name')))
            zone = extra + ' (추가 확인)' if extra else '미확정'
        lines.append(f"| {site} | {network} / {resolver} | {len(r['chain'])-1} | {zone} | {verdict} | {'제3자' if naive else '동일'} |")
    if supplements:
        lines += ['', '### 최종 구역 추가 확인', '',
                  '원래 측정의 SOA 확인 실패는 chains.json에 그대로 남겼다. 아래는 해당 최종 호스트 이름을 나중에 같은 리졸버로 조회한 결과이며, 당시 네트워크에서 성공한 측정으로 소급하지 않는다. 현재 연결 종류는 별도로 확인하지 않았다.',
                  '[추가 SOA 조회 원문과 UTC 시각](zone_supplement.json)', '']
        for s in supplements:
            lines.append(f"- {s['site']} / {s['resolver']}: `{s['final_name']}` → `{s.get('zone', '미확정')}`, 추가 조회 시작 {s['checked_utc']}.")
